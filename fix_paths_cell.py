#!/usr/bin/env python3
"""Fix the data loading cell to use absolute paths."""
import json
...
for i, cell in enumerate(nb['cells']):
    ...
    if 'global_index = initialize_corpus' in source_str:
        ...
        cell['source'] = [
                '# ── SEC Corpus Loading ────────────────────────────────────────────────────────────────────────\n',
            '\n',
            '# Resolve paths to absolute using PROJECT_ROOT\n',
            'chunks_path = (PROJECT_ROOT / CONFIG[\'sec_chunks_path\']).resolve()\n',
            'chroma_path = (PROJECT_ROOT / CONFIG[\'chroma_db_path\']).resolve()\n',
            '\n',
        ...
        print("✓ Updated data loading cell to use absolute paths")
